...
export function renderReferenceSheet({project,borehole,range,sheetIndex=0,sheetTotal=1}){
  ...
  if(b.well?.enabled){const cx=1035,outerL=cx-42,outerR=cx+42,pipeL=cx-9,pipeR=cx+9,st=b.well.screenTopM,sb=b.well.screenBottomM,rb=b.well.riserBottomM??st,wd=b.well.waterDepthM;const topD=range.fromM,endD=Math.min(range.toM,Number(sb??b.totalDepthM||range.toM));const yy=y(topD),zz=y(endD);s+=`<rect x="${outerL}" y="${yy}" width="84" height="${Math.max(1,zz-yy)}" fill="url(#${p}-BENTONITE)" stroke="#222"/><rect x="${outerL+8}" y="${yy}" width="68" height="${Math.max(1,zz-yy)}" fill="url(#${p}-SILICASAND)" stroke="none"/><rect x="${pipeL}" y="${yy}" width="18" height="${Math.max(1,zz-yy)}" fill="#fff" stroke="#111"/>`;
